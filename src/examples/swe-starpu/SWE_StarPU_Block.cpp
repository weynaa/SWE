...
void SWE_StarPU_Block::initScenario(float _offsetX, float _offsetY,
    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            ...
            huv_Block.h(i, j) = i_scenario.getWaterHeight(x, y);
            huv_Block.hu(i, j) = i_scenario.getVeloc_u(x, y) * huv_Block.h(i, j);
            huv_Block.hv(i, j) = i_scenario.getVeloc_v(x, y) * huv_Block.h(i, j);
        }
    }
